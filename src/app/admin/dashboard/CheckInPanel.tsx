...
export default function CheckInPanel() {
  ...
  // Barcode scanners fire characters rapidly (a few ms apart) then send Enter.
  // We debounce on an 80 ms gap as a fallback for scanners that don't send
  // Enter, but only auto-submit if the whole buffer was typed at scanner
  // speed — otherwise a person typing a phone number digit-by-digit would
  // get an incomplete lookup fired the moment they pause past 6 characters.
  const SCAN_SPEED_MS_PER_CHAR = 30;
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!val) {
      typingStart.current = null;
    } else if (typingStart.current === null) {
    ...
    scanTimer.current = setTimeout(() => {
      ...
      const elapsed = typingStart.current !== null ? now - typingStart.current : Infinity;
      const avgMsPerChar = buffered.length > 0 ? elapsed / buffered.length : Infinity;

      if (buffered.length >= 6 && avgMsPerChar < SCAN_SPEED_MS_PER_CHAR) {
        lookup(buffered);
      }
    }, 80);
  }
  ...
  return (
    <div
      className={`flex-1 flex flex-col transition-colors duration-300 ${bg}`}
